...
//Main Function
void plotEffMuCent_v5_pteta_outputtext(std::string filen ="L3_crabbed_1176_wL2fix"){
...
/*  TH2D* hr = new TH2D("hr","hr", 20,0, 100);
  ...
  //set output text file for pt eta phi;
  std::ofstream out1;
  out1.open(("./rawtxt/Kinematics_"+pname+".txt").c_str());
  out1 << "Event # " << "flag " << "pt " << "eta " << "phi " << "centrality " << "deltaR "<< "deltaPt"<< std::endl;
  //Loop over muons
  for(std::vector<std::pair<int, int> >::const_iterator it = evl.begin(); it != evl.end(); it++){
    ...
    const auto particlesraw = recoInfo.getParticlesraw("muon");
    ...
    std::vector<double> vecdR(cEntries, 10.);
    std::vector<double> vecdPt(cEntries,10.);

    //fill
    for( auto recMR : particlesraw ){
      TLorentzVector recomuonraw = recMR.first;
      bool PassID = recMR.second;
      if(PassID){
	out1 << oEvent <<" "<< "RECO_T" <<" "<< recomuonraw.Pt() <<" "<< recomuonraw.Eta()<<" "<< recomuonraw.Phi()<<" "<< centI<<std::endl;
      }
      else if(!PassID){
	out1 << oEvent <<" "<< "RECO_F" <<" "<< recomuonraw.Pt() <<" "<< recomuonraw.Eta()<<" "<< recomuonraw.Phi()<<" "<< centI<<std::endl;
      }
    }
    for( auto recM : particles){
      TLorentzVector recomuon = recM.first;
     // out1 << oEvent <<" "<< "RECO" <<" "<< recomuon.Pt() <<" "<< recomuon.Eta()<<" "<< recomuon.Phi()<<" "<< centI<<std::endl;
      if(centI <= centralcut){
	...
	if( std::abs(onmuon->Eta()) > 2.4 )continue;
	double dRcut = pname.find("L2")!=std::string::npos ? 0.3 : 0.1;
	vecdR[k] = std::min(vecdR[k],onmuon->DeltaR(recomuon));
	vecdPt[k] = std::min(vecdPt[k], std::abs(onmuon->Pt()-recomuon.Pt())/recomuon.Pt());
    ...
    for(int j=0; j < cEntries; j++){
      ...
      if( std::abs(onmuon->Eta()) > 2.4 )continue;
      out1 << oEvent <<" "<< "ONLINE" <<" "<< onmuon->Pt() <<" "<< onmuon->Eta()<<" "<< onmuon->Phi()<<" "<< centI<<" "<<vecdR[j] <<" "<<vecdPt[j] <<std::endl;
      if(centI <= centralcut){
  ...
  //modify plots & draw
  TFile* out = new TFile(("outputratioL3_"+filen+"_part100000_PtEta_MT.root").c_str(),"recreate");
