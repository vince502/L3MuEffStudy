...
//{
//  public:
//    OniaReader(const std::string& path, const bool& buildIndex=false, const bool isMC=true)
//    {
//      setFile(path);
//      setTreeReader("myTree","hionia/myTree");
//      if (buildIndex) {
//	std::cout <<"[INFO] Building index in OniaReader for \"myTree\"" << std::endl;
//	if (isMC){
//	  reader_.at("myTree")->GetTree()->BuildIndex("eventNb");
//	}
//	else reader_.at("myTree")->GetTree()->BuildIndex("runN","eventNb");
//	if (getEntries()>0) { reader_.at("myTree")->GetTree()->LoadTree(0); }
//      }
//      
//      if (isMC) evtI_["runN"]=1;
//      else evtI_["runN"].reset(new TTreeReaderValue<int>(*reader_.at("myTree"), "runN"));
//      evtUL_["eventN"].reset(new TTreeReaderValue<ULong64_t>(*reader_.at("myTree"),"eventNb"));
//  };
//
//  // getters
//  std::pair<Long64_t, Long64_t> getEventNumber() const
//  {
//    if (onDemand_) { loadEntry("myTree"); }
//    if (evtUL_.find("eventN")==evtUL_.end()) { return {0, 0}; }
//    return {*evtI_.at("runN")->Get(), *evtUL_.at("eventN")->Get()};
//  };
//    
